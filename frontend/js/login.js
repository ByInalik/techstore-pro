...
formLogin.addEventListener('submit', async function (evento) {
    ...
    try {
        ...
        if (!respuesta.ok) {
            document.querySelector('#error-login-email').textContent = datos.error || 'Correo o contrasela incorrectos';
            return;
        }
    ...
    formLogin.reset();
        ...
        // 6. Error de red
        document.querySelector('#error-login-email').textContent = 'No se pudo conectar. Verific que npm run dev esté corriendo';
    }
});